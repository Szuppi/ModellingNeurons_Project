...
def make_folder(name, root_dir='C:/Users/local_user/Desktop/Study/VII_semester/Modelling_Neurons/Project_Animations/Phase portrait animations'):
    path = Path(root_dir) / name
    path.mkdir(parents=True, exist_ok=True)
    return path


def setup_phase_portait_ax(show_ax=True, dpi=300):
    """
    Set up the phase portrait axes with a white background and light grid.

    Parameters
    ----------
    show_ax : bool
        Whether to show axes labels/ticks.
    dpi : int
        Figure DPI.
    ...
    if show_ax:
        ...
        # no axes / ticks (for overlay-style videos, if you ever want that)
...
def setup_timeseries_ax(show_ax=True, kind="V", dpi=300):
    """
    Time series axes with white background and grid.
    """
    ...
    if show_ax:
        ...
        ax.tick_params(colors="black", labelsize=20)
        ax.set_xlabel("Time, s", color="black", fontsize=22)

        if kind == "V":
            ax.set_ylabel("$V$, mV", color="black", fontsize=22)
            ax.set_ylim(VOLTAGE_LIMITS[0], VOLTAGE_LIMITS[1])
        else:
            ax.set_ylabel("$n$", color="black", fontsize=22)
            ax.set_ylim(-0.025, 1.025)
    else:
        for side in ax.spines.values():
            side.set_visible(False)
        ax.tick_params(length=0, label1On=False, label2On=False)
        # labels hidden when show_ax=False (if you still want label-less overlays)
...
def animate_timeseries(t, values, kind, save_folder, return_anim=False, to_save=True, color='white', force_ax_lims=None, lw=3, suffix=''):
